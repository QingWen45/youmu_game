...
func run(delta):
	...
	if first_attack and attack_timer > 0.2:
		...
		first_attack = false
		obj.velo.x = obj.ATTACK_DISPLACE * obj.dir_cur * 0.4
		obj.velo = obj.move_and_slide_with_snap( obj.velo, Vector2(0,10), Vector2.UP )

	var dir = Input.get_action_strength("right") - Input.get_action_strength("left")
	if abs(dir) and attack_timer > 1.0:
		fsm.state_next = fsm.states.run
		return

	if Input.is_action_just_pressed("dash") and obj.can_dash() and attack_timer > 1.0:
		obj.dash()
